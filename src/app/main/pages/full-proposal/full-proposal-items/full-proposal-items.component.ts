...
export class FullProposalItemsComponent implements OnInit {
  ...
  openCreateFullProposalItemDialog(): void {
    const dialogRef = this.dialog.open(CreateFullProposalItemComponent, {
      // width: '700px',
  ...
  openEditFullProposalItemDialog(fpItem: any, index: number): void {
    ...
    const fpItemDocID = fpItem.id;

    const dialogRef = this.dialog.open(EditFullProposalItemComponent, {
      // width: '700px',
      width: `${this.createFPItemWidth}px`,
      height: `${this.createFPItemHeight}px`,
      data: { fpItem },
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      if (result) {
        const newFpItem = result;
        newFpItem.id = fpItemDocID;

        this.fpItems[index] = newFpItem;


        this.editFpItemService.editFPItem(newFpItem).subscribe(
          () => {
            this.getFPItems();
          },
          (err) => {
  ...
  openDeleteFullProposalItemDialog(fpItem: any): void {
    const dialogRef = this.dialog.open(DeleteFullProposalItemComponent, {
      // width: '700px',
      width: `${this.createFPItemWidth}px`,
      height: `${this.createFPItemHeight}px`,
      data: { fpItem },
