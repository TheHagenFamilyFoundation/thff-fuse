...
export class AddUsersComponent implements OnInit {
  ...
  dataSourceAllUsers: any;

  // MatTableDataSource<OrganizationData>;
  ...
  onRowClicked(row) {
    ...
    this.dataSourceAllUsers.data.forEach((element, index) => {
      ...
      if (element.id === row.id) {
        ...
        this.allUsersPaginator.length = this.usersCount - 1; // reduce the count after selecting 1
        this.dataSourceAllUsers.paginator = this.allUsersPaginator;
        this.dataSourceAllUsers.sort = this.allUsersSort;
  ...
  applyFilter(filterValue: string) {
    ...
    filteredValue = filteredValue.toLowerCase(); // MatTableDataSource defaults to lowercase matches
    this.dataSourceAllUsers.filter = filteredValue;
  }
