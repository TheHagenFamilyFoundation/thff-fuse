...
import { MatPaginator } from '@angular/material/paginator';// for table
import { MatSort } from '@angular/material/sort';// for table
...
@Component({
  ...
  styleUrls: ['./director-table-full-proposals.component.scss'],
})
export class DirectorTableFullProposalsComponent implements OnInit {
  ...
  openSelectedFPDialog(fp): void {
    const dialogRef = this.dialog.open(DirectorSelectedFPComponent, {
      width: '400px',
      data: { fp, fpID: fp.fpID },
    });
