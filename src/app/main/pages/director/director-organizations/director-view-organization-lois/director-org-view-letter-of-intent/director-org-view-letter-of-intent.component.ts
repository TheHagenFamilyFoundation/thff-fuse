...
@Component({
  ...
  styleUrls: ['./director-org-view-letter-of-intent.component.scss'],
})
export class DirectorOrgViewLetterOfIntentComponent implements OnInit {
  ...
  // organization object retrieved from the loi object
  orgName: string;
      ...
      .subscribe(
        (loi) => {
          ...
          this.getNextLoiService.getNextLOI(nextLOIData).subscribe(
            (nextLoi) => {
              console.log('next loi', nextLoi);

              if (nextLoi) {
                this.Last = false;

                this.nextLOILink = this.loiLink + nextLoi.loiID;
          ...
          this.getPrevLoiService.getPrevLOI(prevLOIData).subscribe(
            (prevLoi) => {
              console.log('prev loi', prevLoi);

              if (prevLoi) {
                this.First = false;

                this.prevLOILink = this.loiLink + prevLoi.loiID;
